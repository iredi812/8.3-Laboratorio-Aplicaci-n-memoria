...
const cambiarBackgroundConDelay = (
...
const pintarParejaEncontradaColorVerde = (
  indiceA: number,
  indiceB: number
): void => {
  ...
  if (
    imageA &&
    imageA instanceof HTMLImageElement &&
    imageB &&
    imageB instanceof HTMLImageElement
  ) {
...
const pintarVolteoHaciaAbajoDeLaPareja = (
  indiceA: number,
  indiceB: number
): void => {
...
const mostrarReversoCartas = (): void => {
  const imagelist = document.getElementsByClassName(
    __CSSCLASNAMES.standardImage
  );
